...
public class Vars {
	...
	public static final float ACTION_ANIMATION_RATE = .1f;
		...
		for(int y = 0; y < text.size; y++){
			...
			for(int x = 0; x< s.length(); x++){
				...
				if(x > max){
					...
					if(y <= text.size - 2){
						if(lastSpace!=0){
							text.set(y + 1, s.substring(lastSpace + 1) + text.get(y + 1));
							s = s.substring(0, lastSpace);
						} else {
							text.set(y + 1, s.substring(max + 1) + text.get(y + 1));
							s = s.substring(0, max);
						}
					} else {
						if(lastSpace!=0){
							tmp.add(s.substring(0, lastSpace));
							tmp.add(s.substring(lastSpace + 1));
						} else {
							tmp.add(s.substring(0, max));
							tmp.add(s.substring(max + 1));
						}
						break;
					}
				}
