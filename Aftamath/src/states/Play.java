...
public class Play extends GameState {
	...
	private int stateType, prevStateType;
	...
	public void create(){
		...
		player = new Player("TestName", "male", "player1", Game.width/2, 158);
...
//		character = scene.getMob("The Crusher");
		character = player;
		cam.setCharacter(character); //set camera focus to the character
		
		setSong(scene.DEFAULT_SONG, 0);
...
//			light -= .1f; rayHandler.setAmbientLight(light);
	...
	private void use(){
		if(player.getPartner().getName() != null){
			...
			//    speak();
			//else
			if(player.getPartner().getState() == NPC.FOLLOWING) {
				...
				if(player.getPartner().getGender().equals("female")) displayText.add("Coming!");
		...
		if(character instanceof Reaper)
			((Reaper) character).warp();
	}
	...
	private void initEntities() {
		...
		for (Entity d : objects){
			d.setPlayState(this);
			d.create();
			if(d instanceof Entity) ((Entity) d).setPlayer(player);
		}
	...
	public BoundedCamera getCam(){ return cam; }
	...
	public void sortObjects(){
		...
		int i = 64;
		while (tmp.size() != objects.size() && i > 0){
			short layer = (short) Math.pow(2, i);
			for(Entity d : objects){
				if (d.getLayer() == layer)
					tmp.add(d);
			}
