...
public class TextBox extends Entity {
	...
	public TextBox(Entity owner, String message, boolean kill) {
		...
		this.y = owner.getPosition().y*Vars.PPM + owner.rw;
...
//		String msg = msgSrc.substring(0, shown);
		...
		if(expanded){
			...
			for(int i = 0; i<text.length; i++){
				...
				GameState.drawString(sb, font, font[0].getRegionWidth(), text[i], 
	...
	public void kill(){
		...
		expanded = false;
		if (!selfKill && owner instanceof NPC)
			((NPC)owner).resetState();
	}
